        ...
        <div className={`demo-shell ${active ? active.mode.toLowerCase() : 'secure'} ${detectionMode === 'real' && operationalState !== 'PROTECTION ACTIVE' ? 'degraded' : ''}`}>
          ...
          <div className="demo-body">
            ...
            <div className="dashboard-wrap">
              ...
              <ForensicWatermark state={threatState} organizationId={runtimeConfig.organizationId || 'demo-org'} deviceId={runtimeConfig.deviceId || 'demo-device'} sessionId={runtimeConfig.sessionId || 'demo-session'}/>
...
function IncidentDrawer({ incident, loading, onClose, onStatus, onAddNote, organizationId }: { incident: NonNullable<ReturnType<typeof useIncidents>['selected']>; loading: boolean; onClose: () => void; onStatus: (status: 'INVESTIGATING' | 'RESOLVED' | 'FALSE_POSITIVE' | 'DISMISSED') => void; onAddNote: (note: string) => void; organizationId: string }) {
  ...
  return <div className="drawer-backdrop" role="dialog" aria-modal="true"><aside className="device-drawer incident-drawer"><div className="drawer-head"><div><span>INCIDENT INVESTIGATION</span><h3>{incident.threat_type} · {incident.severity}</h3></div><button onClick={onClose} aria-label="Close incident details"><X/></button></div>
    ...
    <div className="timeline-panel"><b>Chronological timeline</b>{incident.events.length === 0 ? <span>No timeline events recorded for this incident.</span> : incident.events.map(event => <div className="timeline-event" key={event.id}><time>{formatTime(event.occurred_at)}</time><div><b>{event.event_type.replaceAll('_', ' ')}</b><span>{event.message}</span><small>{event.source} · Risk {formatReported(event.risk_score)}{event.frame_id ? ` · Frame ${event.frame_id}` : ''}</small>{typeof event.metadata.watermark_fingerprint === 'string' && <em>Watermark fingerprint: {event.metadata.watermark_fingerprint.slice(0, 16)}… · deterrence and attribution aid</em>}</div></div>)}</div>
